...
username = os.environ['FOIAXpressUser']
password = os.environ['FOIAXpressPass']
domain = os.environ['FOIAXpressUrl']
table_name = "FOIAXpress"
chunksize = 10000
...
if(not match):
    raise Exception("FOIAXpress Data Connector: Invalid Log Analytics Uri.")
...
class FOIAXpress():
    ...
    def get_report(self, report_type_suffix, report_type_name, params=None):
        try:
            r = requests.get(self.domain + report_type_suffix, auth=(username, password), headers=self.header, params=params)
    ...
    def reports_list(self):
        reports_api_requests_dict = \
            {
                "getuseractions_events": {"api_req": "/GetUserActions", "name": "GetUserActions."},
                "getsserlogins_events": {"api_req": "/GetUserLogins", "name": "GetUserLogins."}
            }
        for api_req_id, api_req_info in reports_api_requests_dict.items():
            ...
            self.get_report(report_type_suffix=api_req, report_type_name=api_req_name, params=api_req_params)
...
def main(mytimer: func.TimerRequest) -> None:
    ...
    sentinel = Sentinel()
    FX.reports_list()
    FOIAXpress_class_vars = vars(FX)
    from_date, to_date = FOIAXpress_class_vars["from_date"], FOIAXpress_class_vars["to_date"]
    results_array = FOIAXpress_class_vars["results_array"]
